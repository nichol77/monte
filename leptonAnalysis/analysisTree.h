...
// from TTree analysisTree/Energy Loss Tree
...
//   Int_t  Cut(Int_t entry);
   ...
   void   FillHistos(char *histFilename=0);
   Bool_t Notify();
...
analysisTree::analysisTree(char *filename)
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
    TTree *tree=0;
...
Int_t analysisTree::GetEntry(Int_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Int_t analysisTree::LoadTree(Int_t entry)
{
...
void analysisTree::Show(Int_t entry)
{
...
/* Int_t analysisTree::Cut(Int_t entry) */
/* { */
