...
void analysisTree::Loop(Double_t selCut)
{
...
void analysisTree::FillHistos(char *histFilename)
{
   if (fChain == 0) return;
   ...
   TFile *fpHists=0;
   if(histFilename) 
       fpHists = new TFile(histFilename,"RECREATE");

   TH1F *histNums[8];
   TH1F *histDists[8];
   TH1F *histTimes[8];
   TH1F *histFirstDist[8];
   TH1F *histFirstTime[8];
   TH1F *histIntTypes[8];
   char histName[80];
   char histTitle[80];
   ...
   for(int i=0;i<8;i++) {
       ...
       histFirstDist[i]= new TH1F(histName,histTitle,250,0,50);
       sprintf(histName,"histFirstTime%d",i);
       histFirstTime[i]= new TH1F(histName,histTitle,250,0,100);
       sprintf(histName,"histIntType%d",i);
       histIntTypes[i]= new TH1F(histName,histTitle,5,-1.5,3.5);
   }
   TH1F *histLastInteraction = new TH1F("histLastInteraction","Last Interaction",7,-0.5,6.5);
   TH1F *histTotalLength = new TH1F("histTotalLength","Total Length",250,0,250);
   TH1F *histTotalEnergyLoss = new TH1F("histTotalEnergyLoss","Total Energy Loss",12000,0,12);
  

   Int_t numEventsAboveCut[8]={0};
   Double_t lastTotalLengthForSEL[8]={0};
   Int_t lastIntType=0;
   Double_t lastTotalLength=0;
   Double_t lastTotalEnergyLoss=0;
   for (Int_t jentry=0; jentry<nentries;jentry++) {
      Int_t ientry = LoadTree(jentry);
      if (ientry < 0) break;
      nb = fChain->GetEntry(jentry);   nbytes += nb;
      if(particleNumber!=lastParticleNumber) {
	  histTotalLength->Fill(lastTotalLength);
	  histTotalEnergyLoss->Fill(TMath::Log10(lastTotalEnergyLoss));
	  if(lastIntType<4) 	      
	      histLastInteraction->Fill(1);
	  else if(lastIntType==4)
	      histLastInteraction->Fill(3);
	  else 
	      histLastInteraction->Fill(5);
	  for(int i=0;i<8;i++) {
...
// 	      }
	      ...
	      if(numEventsAboveCut[i]==1) {
		  histFirstDist[i]->Fill((totalLength-lastTotalLengthForSEL[i])/1e5);
		  histFirstTime[i]->Fill(1e4*(totalLength-lastTotalLengthForSEL[i])/c_light);
	      }
      ...
      lastParticleNumber=particleNumber;
      lastIntType=stepIntType;
      lastTotalEnergyLoss=totalEnergyLoss;
      lastTotalLength=totalLength/1e5;
   }
   for(int i=0;i<8;i++) 
       histNums[i]->Fill(numEventsAboveCut[i]);
