...
void plotRayPaths(Int_t depth) {
  ...
  TH1F *framey=gPad->DrawFrame(0,-2*depth,200,0,histTitle);
  framey->GetXaxis()->SetTitle("Horizontal Distance (m)");
  framey->GetYaxis()->SetTitle("Depth (m)");


  TF1 *fitty = new TF1("fitty",rayPath,0,7000,4);
  ...
  //  fitty->Draw("same");
  for(double angle=TMath::Pi()/80;angle<TMath::Pi()/2;angle+=TMath::Pi()/45) {
  ...
  liney->DrawLine(0,-200,7000,-200);

  can->cd(2);
  TF1 *timey = new TF1("timey",timePath,0,7000,4);
  TH1F *framey=gPad->DrawFrame(0,0,200,1000,histTitle);
  framey->GetXaxis()->SetTitle("Horizontal Distance (m)");
